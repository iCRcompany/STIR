...
*/
  static void upsample_and_fit_scatter_estimate(ProjData& scaled_scatter_proj_data,
...
private:
  ...
  static void apply_mask_in_place(DiscretisedDensity<3, float>&, const MaskingParameters&);
  ...
  shared_ptr<BinNormalisation> get_normalisation_object_sptr(const shared_ptr<BinNormalisation>& combined_norm_sptr) const;

  //! extract the attenuation factors from a combined norm
  shared_ptr<ProjData> get_attenuation_correction_factors_sptr(const shared_ptr<BinNormalisation>& combined_norm_sptr) const;
